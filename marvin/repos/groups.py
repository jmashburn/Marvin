...
class RepositoryGroup(RepositoryGeneric[GroupRead, Groups]):
    ...
    def create_many(self, data: Iterable[GroupRead | dict]) -> list[GroupRead]:
    ...
    def update(self, match_value: str | int | UUID4, new_data: GroupUpdate | dict) -> GroupRead:
        if isinstance(new_data, GroupCreate):
    ...
    def get_by_slug_or_id(self, slug_or_id: str | UUID) -> GroupRead | None:
