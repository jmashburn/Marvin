...
from . import NOT_SET, NotSet # Sentinel for unset parameters
...
class RepositoryGeneric(Generic[Schema, Model, CreateSchema, UpdateSchema]):
    """
    ...
    def _log_exception(self, e: Exception) -> None:
        ...
        """
        self.logger.error(
            f"Error processing query for Repo model={self.model.__name__} schema={self.schema.__name__}"
        )
        self.logger.error(e)

    def _query(
        self, override_schema: type[_MarvinModel] | None = None, with_options: bool = True
    ) -> Select:
        """
    ...
    def multi_query(
        self,
        query_by: dict[str, Any], # Value can be str, bool, int, UUID4 etc.
        start: int = 0,
        limit: int | None = None,
        ...
        if order_by:
            order_attribute = getattr(self.model, order_by, None)
            if order_attribute:
    ...
    def get_one(
        self, value: Any, key: str | None = None, any_case: bool = False, override_schema: type[Schema] | None = None
    ) -> Schema | None:
        """
        ...
        if any_case:
            ...
            model_attribute = getattr(self.model, key_to_use, None)
    ...
    def update(
        self, match_value: Any, new_data: UpdateSchema | dict, match_key: str | None = None
    ) -> Schema:
        """
    ...
    def update_many(self, data: Iterable[UpdateSchema | dict]) -> list[Schema]:
        ...
        for document_update_data in data:
            item_dict = (
                ...
                if isinstance(document_update_data, dict) 
                else document_update_data.model_dump(exclude_unset=True)
            )
    ...
    def page_all(
        ...
        paginated_query, total_count, total_pages = self.add_pagination_to_query(
            base_query, pagination_params
        )
    ...
    def add_pagination_to_query(
        self, query: Select, pagination: PaginationQuery
        ...
        if pagination.query_filter:
            try:
                ...
                query = query_filter_builder.filter_query(
                    query, model=self.model, column_aliases=self.column_aliases
                )
            ...
            for order_by_statement in order_by_fields:
                ...
                try:
                    ...
                    _, model_attr, query_with_joins_if_any = QueryFilterBuilder.get_model_and_model_attr_from_attr_string(
                        field_name, self.model, query=query # Pass current query to allow joins to be added
                    )
                    ...
                    query = self.add_order_attr_to_query(
                        query, model_attr, order_direction, request_query.order_by_null_position
                    )
...
class GroupRepositoryGeneric(RepositoryGeneric[Schema, Model, CreateSchema, UpdateSchema]):
    """
    ...
    def __init__(
        ...
        *, # Keyword-only arguments follow
        ...
        if group_id is NOT_SET:
            ...
            raise ValueError(
                f"group_id must be explicitly set (can be None) for {self.__class__.__name__}, but was NOT_SET."
            )
