...
from marvin.core.security import hash_password, url_safe_token
from marvin.db.models.users.users import AuthMethod
from marvin.repos.all_repositories import get_repositories
from marvin.schemas.user.password import PasswordResetToken
from marvin.services import BaseService
from marvin.services.email import EmailService


class PasswordResetService(BaseService):
    def __init__(self, session: Session) -> None:
        self.db = get_repositories(session, group_id=None)
        super().__init__()

    def generate_reset_token(self, email: str) -> PasswordResetToken | None:
        ...
        save_token = PasswordResetToken(user_id=user.id, token=token)
