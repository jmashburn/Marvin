...
class GroupWebhooksModel(SqlAlchemyBase, BaseMixins):
    ...
    url: Mapped[HttpUrlType] = mapped_column(HttpUrlType, nullable=False, doc="The URL to which the webhook request will be sent.") # Made HttpUrlType non-optional as per nullable=False
    method: Mapped[Method] = mapped_column(SqlAlchemyEnum(Method), default=Method.POST, doc="HTTP method to use for the webhook request (GET or POST). Defaults to POST.") # Used SqlAlchemyEnum

    # New Fields for extended functionality
    webhook_type: Mapped[EventDocumentType | None] = mapped_column(
        SqlAlchemyEnum(EventDocumentType), default=EventDocumentType.GENERIC, doc="Type of the webhook, influencing payload or trigger. Defaults to 'generic'."
    ) # Used SqlAlchemyEnum and updated default
    scheduled_time: Mapped[time | None] = mapped_column(
        Time, default=lambda: datetime.now(timezone.utc).time(), doc="Scheduled time (UTC) for the webhook to run, if it's a scheduled webhook. Defaults to current UTC time on creation."
    )
