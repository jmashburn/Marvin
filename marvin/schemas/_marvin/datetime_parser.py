...
def parse_time(value: time | str | bytes | int | float) -> time:
    ...
    if numeric_value is not None:
        ...
        return (datetime.min.replace(tzinfo=timezone.utc) + timedelta(seconds=numeric_value)).time().replace(tzinfo=None) # Naive time
...
def parse_datetime(value: datetime | str | bytes | int | float) -> datetime:
    ...
    try:
        return datetime(**datetime_params) # Create datetime object
