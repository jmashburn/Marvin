...
@controller(router)
class WebhookReadController(BaseUserController):
    @cached_property
    def repo(self):
    ...
    def mixins(self) -> HttpRepo:
        return HttpRepo[WebhookCreate, WebhookUpdate, WebhookCreate](self.repo, self.logger)

    @router.get("", response_model=WebhookPagination)
    def get_all(self, q: PaginationQuery = Depends(PaginationQuery)):
        response = self.repo.page_all(
            pagination=q,
            override=WebhookRead,
        )
    ...
    def create_one(self, data: WebhookCreate):
    ...
    @router.get("/{item_id}", response_model=WebhookRead)
    ...
    @router.put("/{item_id}", response_model=WebhookRead)
