...
from fastapi import HTTPException, status, APIRouter # APIRouter for instantiation
...
router = UserAPIRouter(prefix="/api-tokens", tags=["Users - API Tokens"]) # Corrected prefix to be more specific
...
class UserApiTokensController(BaseUserController):
    ...
    @router.post("", status_code=status.HTTP_201_CREATED, response_model=LongLiveTokenCreateResponse, summary="Create an API Token")
    ...
    def delete_api_token(self, token_id: UUID4) -> TokenResponseDelete: # Changed token_id type to UUID4
        ...
        if not hasattr(token_to_delete, 'user_id') or token_to_delete.user_id != self.user.id:
            ...
            raise HTTPException(
                ...
                detail="User is not authorized to delete this API token."
            )
