...
from marvin.schemas.user.user import (
    PrivateUser,
    UserCreate,
    UserUpdate,
)  # User Pydantic schemas. UserUpdate might be more appropriate for new_data
...
def _assert_non_admin_user_change_allowed(user_id_being_edited: UUID4, current_user: PrivateUser, new_data: UserCreate | UserUpdate) -> None:
    """
    ...
    for attr_name in permission_attrs:
        ...
        if hasattr(new_data, attr_name) and getattr(current_user, attr_name, None) != getattr(new_data, attr_name):
    ...
    elif hasattr(new_data, "group") and new_data.group is not None and hasattr(current_user, "group") and current_user.group != new_data.group:
...
def assert_user_change_allowed(user_id_being_edited: UUID4, current_user: PrivateUser, new_data: UserCreate | UserUpdate) -> None:
    """
