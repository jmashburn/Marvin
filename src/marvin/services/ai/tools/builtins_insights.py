...
@register_tool(
    ...
    description="List recent AI operation executions in this workspace (operation, status, model, tokens, cost, timing). Optionally filter by status, operation slug, or entity_type. Use to see what AI has run and how much it cost.",
...
@register_tool(
    ...
    description="Get one AI execution by id, including its input and output JSON (the full record). Use after list_ai_executions to inspect what a run produced or why it failed.",
...
def get_ai_execution(ctx: ToolContext, args: dict) -> str:
    ...
    return _dump({
        "id": str(r.id), "operation": r.operation_slug, "status": r.status,
        "provider": r.provider_type, "model": r.model_id, "triggerType": r.trigger_type,
        "entityType": r.entity_type, "entityId": str(r.entity_id) if r.entity_id else None,
        "input": r.input_json, "output": r.output_json,
        "promptTokens": r.prompt_tokens, "completionTokens": r.completion_tokens,
        "totalTokens": r.total_tokens, "estimatedCostUsd": r.estimated_cost_usd,
        "durationMs": r.duration_ms, "error": r.error_message,
...
@register_tool(
    ...
    description="Get this workspace's AI policy (enabled, credential mode, approval mode, provider/model, invocation sources). Use to explain why AI is on/off or a source is blocked. Never returns secrets.",
    input_schema={"type": "object", "properties": {}},
    min_role=ROLE_VIEWER,
)
...
@register_tool(
    name="list_events",
    description="List recent events from the workspace audit log (entry/asset/collection lifecycle, automations, …). Optionally filter by event_type, entity_type, or correlation_id (to follow one causal chain). Use for 'what happened recently' or to trace a cascade.",
    input_schema={"type": "object", "properties": {
...
def list_events(ctx: ToolContext, args: dict) -> str:
    ...
    out = [{
        ...
        "entityType": e.entity_type, "entityId": str(e.entity_id) if e.entity_id else None,
        "correlationId": e.correlation_id, "title": e.message_title,
    } for e in events]
...
@register_tool(
    ...
    description="Get the full event history for one entity (entry, asset, …) by id — every lifecycle event, newest first. Use to answer 'what happened to this entry'.",
    input_schema={"type": "object", "properties": {
...
@register_tool(
    ...
    description="List the workspace's scheduled tasks (name, slug, schedule, enabled, last run + status, next run). Use for 'what's scheduled' or 'is task X enabled'.",
    input_schema={"type": "object", "properties": {}},
    min_role=ROLE_VIEWER,
)
def list_scheduled_tasks(ctx: ToolContext, _args: dict) -> str:
    ...
    out = [{
        "id": str(t.id), "name": t.name, "slug": t.slug, "taskType": t.task_type,
        "enabled": t.enabled, "scheduleType": t.schedule_type,
        "lastRunAt": t.last_run_at, "lastStatus": t.last_status, "nextRunAt": t.next_run_at,
...
@register_tool(
    ...
    description="Get recent execution history for one scheduled task by id or slug (each run's status, duration, error). Use to see whether a task is succeeding.",
    input_schema={"type": "object", "properties": {
...
def get_scheduled_task_history(ctx: ToolContext, args: dict) -> str:
    ...
    out = [{
        "executedAt": r.executed_at, "status": r.status, "durationMs": r.duration_ms,
        "error": r.error_message, "retryAttempt": r.retry_attempt,
    } for r in runs]
