...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("is_rendered", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("created_at", sa.DateTime(), nullable=True),
        sa.Column("update_at", sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(["group_id"], ["groups.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    ...
    # users must exist before event_log (event_log FKs users.id); Postgres enforces FK-target
    # ordering at create time, unlike SQLite. users only depends on groups (already created above).
    op.create_table(
        "users",
        sa.Column("id", marvin.db.migration_types.GUID(), nullable=False),
