...
def include_object(object: Any, name: str | None, type_: str, reflected: bool, compare_to: Any | None) -> bool:
    ...
    if type_ == "table":
        ...
        # Don't include indexes that are not in the model. Compare against index *names* —
        # `table.indexes` is a set of Index objects, so testing a str against it is always
        # False and would silently drop every index from autogenerated migrations.
        return name in {ix.name for ix in target_metadata.tables[object.table.name].indexes}
    return True
